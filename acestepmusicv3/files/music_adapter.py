...
def _draft_query(task: dict[str, Any], attempt: int) -> str:
    ...
    anchors = {
        "zh": (
            ...
            "普通话歌曲。内部歌词优先按每句[zh]加声调数字拼音生成，也可直接写自然汉字。必须有完整Verse、Chorus及连贯情绪发展，不写占位词或循环乱码。主题与风格：",
            "以中文演唱。内部合成稿请使用每句[zh]开头的声调数字汉语拼音，或自然中文汉字；歌词应有具体故事、段落发展和有意义的副歌。主题与风格：",
        ),
        "yue": (
            "粤语歌曲。内部歌词优先使用ACE标准发音脚本：每句以[yue]开头，只写带1-5声调数字的粤语音节；也可直接写自然粤语汉字。创作完整具体的情节和情绪发展。主题与风格：",
            "广东话歌曲。内部歌词优先按每句[yue]加声调数字音节生成，也可直接写自然粤语汉字。必须有完整段落和连贯情绪发展。主题与风格：",
            "以粤语演唱。内部合成稿请使用每句[yue]开头的声调数字音节，或自然粤语汉字；歌词应有具体故事和有意义的副歌。主题与风格：",
        ),
    }
