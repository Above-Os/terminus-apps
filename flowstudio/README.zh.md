...
当前 Chart 版本：**0.3.50**（须与 `Chart.yaml` / `OlaresManifest.yaml` 一致）。
...
Manifest 中的 `upgradeDescription` 跟随 `spec.versionName`（应用发布版本），目前对应 0.3.50。
本 Chart 使用镜像 `flowstudio:0.3.50` 与 `engine-1.0.7`（安装期不预取 ROCm）。
